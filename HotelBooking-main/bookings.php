    ...
    <div class="row">
        ...
        while($data = mysqli_fetch_assoc($result))
        {
            ...
            if($data['booking_status']=='booked')
            {
                ...
                if($data['arrival']==1)
                {
                    ...
                    if($data['rate_review']==0){
                        $btn.="<button type='button' onclick='review_room($data[booking_id],$data[room_id])' data-bs-toggle='modal' data-bs-target='#reviewModal' class='btn btn-dark btn-sm shadow-none ms-2'>Đánh giá & Nhận xét</button>";
                    }
                }
                else{
            ...
            else if($data['booking_status']=='cancelled')
            {
                ...
                if($data['refund']==0){
                    $btn="<span class='badge bg-primary'>Đang hoàn tiền, vui lòng đợi trong giây lát</span>";
                }
                else{
                    $btn="<a href='generate_pdf.php?gen_pdf&id=$data[booking_id]' class='btn btn-dark btn-sm shadow-none'>Tải PDF</a>";
                }
            ...
            else
            {
                ...
                $btn="<a href='generate_pdf.php?gen_pdf&id=$data[booking_id]' class='btn btn-dark btn-sm shadow-none'>Tái PDF</a>";
            }
            ...
            <form id="review-form">
                <div class="modal-header">
                    <h5 class="modal-title d-flex align-items-center">
                        <i class="bi bi-chat-square-heart-fill fs-3 me-2"></i> Đánh giá & Nhận xét
                    </h5>
...
if(isset($_GET['cancel_status'])){
    alert('success','Hùy đặt phòng thành công!');
}
...
<script>
    ...
    {
        if(confirm('Bạn có muốn hủy đặt phòng không?'))
        {
